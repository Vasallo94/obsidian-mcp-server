...
- **Destructive writes (`notes.delete`, `notes.replace`, `notes.apply_replace`, `rules.add`, and `canvas.remove_*`)** need an explicit `confirm=True` argument. Without it the server returns a Spanish hint and does nothing — re-invoke with `confirm=True`. Your host shows its own permission prompt before the call runs, so that is the human approval surface (no `elicit()` capability required). Don't fall back to `rm` or full-file `notes.patch`.
